...
성형·피부 크로스보더 클리닉용 **2축 AI 시스템** (순차 빌드): **모듈 1 = 술후 안전 트리아지**(환자 텍스트+사진 → 정상은 다국어 안심 / 불확실·이상징후는 기권→의사 에스컬레이션, 명제 = "AI 정확도가 아니라 *못 믿을 때 사람에게 넘기는 능력*", 측정된 안전) + **모듈 2 = 마케팅·평판 인텔**(Scrapling 크롤 → 경쟁 마케팅 레퍼런스·성공요인 가설·평판/상대순위, refradar 재현). 통합 = MSO 루프(사후케어→평판→유치). **M1 먼저 완결 → M2**, 각 축 단일기능.
