...
def build() -> Path:
    ...
    our = d.get("our_clinic_id")
    rep_rows = "".join(
        f"<tr><td>{r['rank']}</td><td>{_esc(r['clinic_id'])}{' ⭐우리(데모)' if r['clinic_id']==our else ''}</td><td>{r['composite']}</td>"
        f"<td>{r['review_count']}</td><td>긍{r['sentiment_dist'].get('긍정',0)}·중{r['sentiment_dist'].get('중립',0)}·부{r['sentiment_dist'].get('부정',0)}</td>"
        f"<td>{_esc(' · '.join(r.get('top_topics', [])))}</td></tr>"
        for r in rep
    )
    topic_chips = "".join(
        f"<span class='chip'>{_esc(t)} <b>{n}</b></span>" for t, n in (d.get("topic_summary") or {}).items()
    )
    ref_rows = "".join(
...
.chip{{display:inline-block;background:#0d1520;border:1px solid #2a3b4d;border-radius:999px;padding:2px 10px;margin:3px;font-size:12px;color:#9fb0c0;}}
</style></head><body>
...
<div class="wrap">
  ...
  <div class="card" style="grid-column:1/-1;"><h3>평판 상대순위 (투명 지수 · 수집셋 내) · 강약점 토픽</h3>
    <div style="margin-bottom:8px;">전체 토픽 신호: {topic_chips or '<span class=note>토픽 없음</span>'}</div>
    <table><tr><th>순위</th><th>클리닉</th><th>composite</th><th>언급</th><th>감성</th><th>토픽(강·약점)</th></tr>{rep_rows}</table>
    <div class="formula">{_esc(d['formula'])}</div>
    <div class="note">{_esc(sparse_note)}</div>
    <div class="note">수집시각 {_esc(d.get('collected_at'))} · {_esc(d['collection_note'])}</div></div>
</div></body></html>"""
