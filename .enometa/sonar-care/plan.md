...
- [ ] **AC-0**: 프로젝트 스캐폴딩 + 설정
  - [x] **AC-0.1**: `pyproject.toml`(또는 `requirements.txt`) 신설 — anthropic·pydantic>=2·uqlm·mapie·deepeval·fastapi·jinja2·pyyaml·numpy·matplotlib 명시 + `src/sonar_care/__init__.py` 패키지 골격
  - [x] **AC-0.2**: `src/sonar_care/config.py` — env(vault)에서 API 키 로드 + Claude 클라이언트 팩토리(uqlm용 LangChain `ChatAnthropic` 래퍼 포함)
